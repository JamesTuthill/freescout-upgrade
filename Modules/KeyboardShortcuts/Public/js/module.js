...
function ksBind(key, callback)
{
	$(document).on('keyup', function(e) {
		...
		if (!e.target
			|| $(e.target).is(':input')
			|| ($(e.target).attr('contentEditable') == 'true' && key != ks_keys['enter'])
			|| (e.ctrlKey && key != ks_keys['enter'])
			|| e.altKey
			|| e.shiftKey
			|| e.metaKey
		...
		//console.log(e.which);
		if (e.which == key) {
			if (key == ks_keys['enter'] && !e.ctrlKey) {
				return;
			}
			callback();
		}
...
{
	// Search
	ksBind(ks_keys['slash'], function() {
		$('#search-dt').not('[aria-expanded=true]').trigger('click');
	});
}