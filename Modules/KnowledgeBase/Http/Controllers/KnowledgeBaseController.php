...
class KnowledgeBaseController extends Controller
{
    ...
    public function settingsSave(Request $request, $mailbox_id)
    {
        ...
        if (!empty($request->kb_action) && $request->kb_action == 'save_import') {

            $settings = $request->settings;

            if (!$request->hasFile('import_file')) {
                $errors['import_file'] = __('Please select a file to import');

                \Session::flash('flash_error_floating', $errors['import_file']);
            ...
            if ($mimeType != 'text/csv') {
                $errors['import_file'] = __('Invalid file format');
            ...
            if (!$file = fopen($path, "r")) {
                $errors['import_file'] = __('Failed to open file');
            ...
            if (isset($request->settings['overwriteOrAmmend']) && $request->settings['overwriteOrAmmend'] == 'overwrite') {
                $deletingArticles = \KbArticle::where('mailbox_id', $mailbox_id)->pluck('id');

                \KbArticle::where('mailbox_id', $mailbox_id)->delete();
                \KbCategory::where('mailbox_id', $mailbox_id)->delete();
                KbArticleKbCategory::whereIn('kb_article_id', $deletingArticles)->delete();
            }

            $categories = \KbCategory::getTreeWithHierarchy($mailbox_id);

            while (($data = fgetcsv($file)) !== false) {
                if (
                    !isset($data[0], $data[3], $data[4])
                    && !strlen($data[0])
                    && !strlen($data[3])
                    && !strlen($data[4])
                ) {
                ...
                $article->fill([
                    ...
                    'title' => $title,
                    'text' => chatGPT::formatTextIntoHtml($text),
                ...
                $categoryId = array_search(strtolower($data[0]), array_column($categories, 'name', 'id'));

                if (!$categoryId) {
                    $newCategory = new \KbCategory();
                    ...
                    $newCategory->expand = false;

                    $newCategory->save();

                    $categoryId = $newCategory->id;

                    $categories[$categoryId] = (object) [
                        "id" => $categoryId,
                ...
                if (isset($data[1]) && strlen($data[1]) > 0 && $categoryId) {
                    $subCategoryId = array_search(
                        strtolower($data[1]),
                        array_column($categories[$categoryId]->subcategories, 'name', 'id')
                    );

                    if (!$subCategoryId) {
                        $newSubCategory = new \KbCategory();
                        ...
                        $newSubCategory->expand = false;

                        $newSubCategory->save();

                        $subCategoryId = $newSubCategory->id;

                        $categories[$categoryId]->subcategories[$subCategoryId] = (object) [
                            "id" => $subCategoryId,
                ...
                if (isset($data[2]) && strlen($data[2]) > 0 && $categoryId && $subCategoryId) {
                    $subSubCategoryId = array_search(
                        strtolower($data[2]),
                        array_column($categories[$categoryId]->subcategories[$subCategoryId]->subcategories, 'name', 'id')
                    );

                    if (!$subSubCategoryId) {
                        $newSubSubCategory = new \KbCategory();

                        $newSubSubCategory->fill([
                            'mailbox_id' => $mailbox_id,
                            'name' => $data[2],
                            'kb_category_id' => $subCategoryId
                        ]);

                        $newSubSubCategory->expand = false;

                        $newSubSubCategory->save();

                        $subSubCategoryId = $newSubSubCategory->id;

                        $categories[$categoryId]->subcategories[$subCategoryId]->subcategories[] = (object) [
                            "id" => $subSubCategoryId,
                ...
                if ($subSubCategoryId) {
                    $selectedCategories = [$subSubCategoryId];
                ...
                $article->categories()->sync(array_filter($selectedCategories));
            }

            \Session::flash('flash_success_floating', __('Knowledge base imported'));
        }
