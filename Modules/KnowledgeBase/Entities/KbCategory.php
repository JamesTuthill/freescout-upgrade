...
class KbCategory extends Model
{
    ...
    public static function getAllCached($mailbox_id = null/*, $check_visibility = false*/)
    {
        ...
        $categories = \Eventy::filter('knowledgebase.all_categories', $categories);
    ...
    public static function getTreeWithHierarchy($mailbox_id){
        $categories = self::getTreeAsList($mailbox_id);
        ...
        array_walk($categories, function ($category) use (&$categoryIds) {
            $details = [];

            $details["id"] = $category->id;
            $details["name"] = strtolower($category->name);
            $details["subcategories"] = [];

            //Top Level Category
            if (!isset($category->kb_category_id)) {
                return $categoryIds[$category->id] = (object) $details;
            }

            //Sub Category
            if (isset($categoryIds[$category->kb_category_id])) {
                return $categoryIds[$category->kb_category_id]->subcategories[$category->id] = (object) $details;
            }

            //Sub Sub Category
            foreach ($categoryIds as $categoryId) {
                if (isset($categoryId->subcategories)) {
                    foreach ($categoryId->subcategories as $subCategory) {
                        if ($subCategory->id == $category->kb_category_id) {
                            return $subCategory->subcategories[$category->id] = (object) $details;
                        }
                    }
                }
            }
    ...
    {
        $visible = \Eventy::filter('knowledgebase.check_visibility', -1, $this);
