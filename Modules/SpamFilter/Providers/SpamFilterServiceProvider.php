...
class SpamFilterServiceProvider extends ServiceProvider
{
    ...
    public function hooks()
    {
        ...
        \Eventy::addFilter('settings.section_settings', function($settings, $section) {
            ...
            $settings['spamfilter.sender'] = base64_decode(config('spamfilter.sender'));
        ...
        \Eventy::addFilter('conversation.created_by_customer', function($conversation, $thread, $customer) {
            ...
            // Process only email conversations.
            ...
            // Filter by subject.
            if (!$is_spam && $subject_filter = base64_decode(config('spamfilter.subject'))) {
                // Do not mark autoresponders as spam.
                if (!$thread->isAutoResponder()) {
                    $subject_filters = preg_split("/\r\n|\n|\r/", $subject_filter);
                    foreach ($subject_filters as $subject_filter) {
            ...
            // Filter by body.
            if (!$is_spam && $body_filter = base64_decode(config('spamfilter.body'))) {
                // Do not mark autoresponders as spam.
                if (!$thread->isAutoResponder()) {
                    $body_filters = preg_split("/\r\n|\n|\r/", $body_filter);
                    $plain_body = strip_tags($thread->body ?? '');
                    foreach ($body_filters as $body_filter) {
            ...
            // Filter by sender.
            if (!$is_spam && $sender_email = base64_decode(config('spamfilter.sender'))) {
                // Do not mark autoresponders as spam.
                if (!$thread->isAutoResponder()) {
                    $sender_filters = preg_split("/\r\n|\n|\r/", $sender_email);
                    foreach ($sender_filters as $sender_filter) {
                        $sender_regex = preg_quote($sender_filter);
                        // Unquote \*.
                        $sender_regex = str_replace('\*', '.*', $sender_regex);
        ...
        // When status changing to Active on customer reply.
        \Eventy::addFilter('conversation.status_changing', function($new_status, $conversation) {
            $spam_status = self::getCustomerSpamStatus($conversation->customer->spam_status, $conversation->mailbox_id);

            // Do not change conversation status to Active
            // if blacklisted customer or domain replied.
            switch ($spam_status['spam_status']) {
                case self::SPAM_STATUS_DOMAIN_BLACKLISTED:
                case self::SPAM_STATUS_EMAIL_BLACKLISTED_HARD:
                case self::SPAM_STATUS_EMAIL_BLACKLISTED_SOFT:
