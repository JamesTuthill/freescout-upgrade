...
function initSavedReplies()
{
	$(document).ready(function() {
		...
		$('.saved-reply-save').click(function(e) {
	    	...
	    	if ($('#saved-reply-'+saved_reply_id+' :input[name="global"]:first').is(':checked')) {
	    		var glob = 1;
	    	} else {
	    		var glob = 0;
	    	}
...
function initNewSavedReply(jmodal)
{
	$(document).ready(function(){
		...
		$('.modal-content .new-saved-reply-save:first').click(function(e) {
	    	...
	    	var modal_content = $(this).parents('.modal-content:first');

	    	var name = modal_content.children().find(':input[name="name"]').val();
	    	var text = modal_content.children().find(':input[name="text"]').val();
	    	var attachments_all = srMapInputToArray(modal_content.children().find(':input[name="attachments_all[]"]'));
	    	var attachments = srMapInputToArray(modal_content.children().find(':input[name="attachments[]"]'));
	    	if (modal_content.children().find(':input[name="global"]:first').is(':checked')) {
	    		var glob = 1;
	    	} else {
	    		var glob = 0;
	    	}
