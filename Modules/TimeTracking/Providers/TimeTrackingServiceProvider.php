...
class TimeTrackingServiceProvider extends ServiceProvider
{
    ...
    public function hooks()
    {
        ...
        \Eventy::addAction('conversation.after_subject_block', function($conversation, $mailbox) {
            ...
            $allow_add = $options['timetracking.allow_add'];
            ...
            } elseif ($options['timetracking.mode'] == self::MODE_MANUAL) {
                $paused = true;
            }
                    ...
                    <?php if ($show_timer): ?>
                        ...
                        <?php if ($interactive): ?>
                            <span class="tt-buttons">
                                ...
                                <?php if ($allow_add): ?>
                                    <button type="button" id="timetr-add" class="btn btn-default btn-sm" data-toggle="tooltip" title="<?php echo __('Submit Time'); ?>" data-loading-text="..."><i class="glyphicon glyphicon-plus"></i></button>
                                <?php endif ?>
                                <?php if ($allow_reset): ?>
                        ...
                        <div id="timelogs" class="hidden">
                            <table class="table">
                                ...
                                <?php foreach ($timelogs as $timelog): ?>
                                    <tr data-timelog-id="<?php echo $timelog->id ?>">
                                        <td>
                                            <button type="button" class="btn btn-<?php echo Conversation::$status_classes[$timelog->conversation_status] ?> btn-light btn-xs timetr-status-btn" readonly="readonly" data-toggle="tooltip" title="<?php echo Conversation::statusCodeToName($timelog->conversation_status); ?>"><i class="glyphicon glyphicon-<?php echo Conversation::$status_icons[$timelog->conversation_status] ?>"></i></button>
                                        </td>
                                        <td>
                                            <?php echo htmlspecialchars($timelog->user->getFullName()); ?>
                                        </td>
                                        <td>
                                            <span class="timetr-log-value" data-time-spent="<?php echo $timelog->time_spent; ?>" data-toggle="tooltip" data-html="true" title="<?php echo \App\User::dateFormat($timelog->created_at); ?><?php if ($timelog->userCanEdit()): ?>&lt;br/&gt;<?php echo __('Click to edit'); ?><?php endif ?>">
                                                <?php echo self::formatTime($timelog->time_spent); ?> <small class="glyphicon glyphicon-info-sign text-help"></small>
                                            </span>
                                        </td>
                                    </tr>
                                <?php endforeach ?>
        ...
        // Show time icon in conversations list.
     ...
     */
    public static function startTimer($conversation, $forced = false)
    {
        if (!$conversation->user_id 
            || in_array($conversation->status, [Conversation::STATUS_CLOSED, Conversation::STATUS_SPAM])
        ...
        // User has to start time manually.
        if (!$forced && \Option::get('timetracking.mode') == self::MODE_MANUAL) {
            return false;
        }
