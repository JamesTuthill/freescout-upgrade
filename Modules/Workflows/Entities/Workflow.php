...
class Workflow extends Model
{
    ...
    public function checkConditions($conversation, $action = '')
    {
        ...
        // Do not trigger workflows while the conversation is in Draft state.
                ...
                switch ($row['type']) {
                    ...
                    case 'user_reply':
                        ...
                        if ($conversation->last_reply_from != Conversation::PERSON_USER) {
                            // Try to find last user message in the conversation.
                            $last_user_thread = self::getLastUserThread($conversation);
                            if (!$last_user_thread) {
                                continue 2;
                            }
                            $last_reply_at = $last_user_thread->created_at;
                            // if (!$last_user_thread && $operator == 'not_in_last') {
                            //     $or_true = true;
                            // } else {
                            //     if (!$last_user_thread) {
                            //         continue 2;
                        ...
                        if ($last_reply_at) {
                            if ($operator == 'in_last') {
                                $or_true = ($last_reply_at > self::subTime($now, $metric, $number));
                            } else {
                                // not_in_last
                                $or_true = ($last_reply_at < self::subTime($now, $metric, $number));
                            }
                    ...
                    case 'customer_reply':
                        ...
                        if ($conversation->last_reply_from != Conversation::PERSON_CUSTOMER) {
                            // Try to find last customer message in the conversation.
                            $last_customer_thread = self::getLastCustomerThread($conversation);
                            if (!$last_customer_thread) {
                                continue 2;
                            }
                            $last_reply_at = $last_customer_thread->created_at;
                            // if (!$last_customer_thread && $operator == 'not_in_last') {
                            //     $or_true = true;
                            // } else {
                            //     if (!$last_customer_thread) {
                            //         continue 2;
    ...
    public static function getLastCustomerThread($conversation)
    {
        if (empty(self::$cond_last_thread[$conversation->id][Thread::TYPE_CUSTOMER])) {
            $last_customer_thread = $conversation->getLastThread([Thread::TYPE_CUSTOMER]);
            self::$cond_last_thread[$conversation->id][Thread::TYPE_CUSTOMER] = $last_customer_thread;
        ...
        return $last_customer_thread;
    }

    public static function getLastUserThread($conversation)
    {
        if (empty(self::$cond_last_thread[$conversation->id][Thread::TYPE_MESSAGE])) {
            $last_user_thread = $conversation->getLastThread([Thread::TYPE_MESSAGE]);
            self::$cond_last_thread[$conversation->id][Thread::TYPE_MESSAGE] = $last_user_thread;
        ...
        return $last_user_thread;
    }

    public static function getLastThreadWithoutWf($conversation, $types)
    {
        ...
        $wf_user_id = self::getUser()->id;

        $query->where(function ($q) use ($wf_user_id) {
                    $q->where('created_by_user_id', null)
                        ->orWhere('created_by_user_id', '!=', $wf_user_id);
                });
