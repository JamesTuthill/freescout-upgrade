...
class WorkflowsServiceProvider extends ServiceProvider
{
    ...
    public function hooks()
    {
        ...
        \Eventy::addAction('conversation.user_forwarded', function($conversation, $thread, $forwarded_conversation, $forwarded_thread) {
            //if ($forwarded_thread->created_by_user_id != Workflow::getUser()->id) {
            Workflow::runAutomaticForConversation($forwarded_conversation, 'conversation.user_forwarded');
            //}
        }, 20, 4);
            ...
            if ($workflow) {
                ...
                foreach ($workflow->actions as $ands) {
                    foreach ($ands as $action) {
                        if (in_array($action['type'], ['email_customer', 'reply'])) {
        ...
        // Set the subject for "Email the customer" action.
        \Eventy::addFilter('email.reply_to_customer.subject', function($subject, $conversation, $last_thread) {
            if (!empty($last_thread) && !empty($last_thread->meta['wf_subj'])) {
                $subject = $last_thread->meta['wf_subj'];
            }
            // If workflow contains multiple "Email the customer" action it's impossible
            // to determine one which created the thread.
            // if (!empty($last_thread) && $last_thread->created_by_user_id == \Workflow::getUser()->id) {
            //     $workflow = \Workflow::getThreadWorkflow($last_thread);
            //     if ($workflow) {
            //         // Find an "Email the customer" action.
