    ...
    public function hooks()
    {
        ...
        \Eventy::addAction('search.display_filters', function($filters, $filters_data) {
            ?>
                <div class="col-sm-6 form-group <?php if (isset($filters['tag'])): ?> active <?php endif ?>" data-filter="tag">
                    ...
                    <input type="text" name="f[tag]" value="<?php echo htmlspecialchars($filters['tag'] ?? '') ?>" class="form-control" <?php if (empty($filters['tag'])): ?> disabled <?php endif ?>>
                </div>
            <?php
        }, 20, 2);
        ...
        \Eventy::addFilter('workflows.conditions_config', function($conditions) {
            $conditions['conversation']['items']['tag'] = [
                ...
                'triggers' => [
                    'tag.attached',
                    // To let "does not contain" condition work.
                    'tag.detached',
                    // To run workflow conditions check with
                    // only one Tag related condition.
        ...
        \Eventy::addAction('conversations.before_delete_forever', function($conversation_ids) {
            ...
            foreach ($conversations_tags as $conversation_tag) {
                ...
                // https://github.com/freescout-helpdesk/freescout/issues/3279
                if ($tag) {
                    $tag->counter--;
                    $tag->save();
                }
        ...
        // Merge tags when merging conversations.
        \Eventy::addAction('conversation.merged', function($conv1, $conv2) {
            // Copy tags from conv2 to conv1.
            $tags = \Tag::conversationTags($conv2);
            foreach ($tags as $tag) {
                $tag->attachToConversation($conv1->id);
            }
        }, 20, 2);
    }
